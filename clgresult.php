...
if(isset($_SESSION['clgname'])){
    ...
    echo "<br><br><br><br>";
$select=$connect->query("select Name from poll where (College_Name='$_SESSION[clgname]')");
    ...
    while($row=mysqli_fetch_assoc($select)){
        ...
        if($sql->num_rows!=0){
            ...
            while($row2=mysqli_fetch_assoc($sql2)){
                $pollname=$poll.$_SESSION['clgname'];
                $cand=$row2['poll'];
    ...
    <nav class='navbar navbar-expand-lg navbar-light bg-warning fixed-top'>
            ...
            <center>
                <form action="printresult.php" method="post">
                    <input type="submit" class="btn btn-outline-success" name="print" value="Print Result">
                </form>
            </center>
            <?php
