...
if(isset($_POST['candbtn'])){
    $cand=$_POST['candbtn'];
    $table=$_SESSION['poll'];
    $sql=$connect->query("delete from $table where Name='$cand'");
...
$table=$_SESSION['poll'];
$check=$connect->query("select * from $table");
if($check->num_rows!=0){
    ...
    <div class="col-sm-3 col-6">
        <center>
            <form action="" method="post">
                <input type="submit" class="btn btn-outline-dark" name="candbtn" value='<?php echo $res["Name"];?>' onclick="return confirm('Are you sure? Do You want to delete <?php echo $res["Name"];?>')">
            </form>
   <label><?php echo $res['Dept']."<br>Year:".$res['Year']?></label>
        </center>
   </div>
